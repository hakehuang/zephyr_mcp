...
@mcp.tool()
def test_git_connection(repo_url: str, username: str = None, password: str = None, project_dir: str = None) -> Dict[str, Any]:
    """
    ...
    if username and password:
        ...
        try:
            ...
            test_url = urlunparse((parsed.scheme, netloc, parsed.path, parsed.params, parsed.query, parsed.fragment))
    ...
    try:
        # 执行git ls-remote命令
        process = subprocess.run(cmd, cwd=cwd, env=env, capture_output=True, text=True, timeout=30)  # 设置超时以防止无限等待
