    ...
    try:
        ...
        if os.path.exists(workspace_path):
            if force:
                ...
                return {"status": "error", "message": f"Directory {workspace_path} already exists. Use force=True to overwrite."}

        # Determine OS and set up environment accordingly
        current_os = platform.system().lower()
        
        if current_os == "windows":
            result = _setup_windows_environment(workspace_path, zephyr_version, install_sdk, sdk_version, platforms)
        elif current_os == "linux":
            result = _setup_linux_environment(workspace_path, zephyr_version, install_sdk, sdk_version, platforms)
        elif current_os == "darwin":
            result = _setup_macos_environment(workspace_path, zephyr_version, install_sdk, sdk_version, platforms)
        else:
            return {"status": "error", "message": f"Unsupported operating system: {current_os}"}
        ...
        return {"status": "error", "message": f"Error setting up Zephyr environment: {str(e)}"}
...
def _setup_windows_environment(
    ...
    try:
        ...
        if not vs_tools_installed:
            ...
            print("It's recommended to install Visual Studio Build Tools for C++ development.")
        ...
        print("Installing West tool with required dependencies in virtual environment...")
        ...
        try:
            if zephyr_version == "latest":
                subprocess.check_call([venv_west, "init", "-m", "https://github.com/zephyrproject-rtos/zephyr", "."])
            else:
                subprocess.check_call([venv_west, "init", "-m", "https://github.com/zephyrproject-rtos/zephyr", "--mr", zephyr_version, "."])
            ...
            if "http_proxy" in env or "https_proxy" in env:
                ...
                if zephyr_version == "latest":
                    subprocess.check_call([venv_west, "init", "-m", "https://github.com/zephyrproject-rtos/zephyr", "."], env=env)
                else:
                    subprocess.check_call([venv_west, "init", "-m", "https://github.com/zephyrproject-rtos/zephyr", "--mr", zephyr_version, "."], env=env)
            else:
                raise
        ...
        requirements_file = os.path.join(workspace_path, "zephyr", "scripts", "requirements.txt")
        if os.path.exists(requirements_file):
            subprocess.check_call([venv_pip, "install", "-r", requirements_file])
        else:
            return {"status": "error", "message": f"Requirements file not found: {requirements_file}"}
        ...
        return {"status": "success", "message": "Zephyr environment set up successfully on Windows", "visual_studio_tools": vs_tools_installed, "venv_path": venv_path}
        
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Command failed: {str(e)}", "suggestions": ["Check your internet connection", "Ensure you have administrator privileges", "Verify all dependencies are installed correctly"]}
    except Exception as e:
        return {"status": "error", "message": f"Windows setup failed: {str(e)}", "suggestions": ["Try running with administrator privileges", "Check available disk space", "Ensure Python is properly installed"]}
...
def _setup_linux_environment(
    ...
    try:
        ...
        return {"status": "success", "message": "Zephyr environment set up successfully on Linux", "venv_path": venv_path}
        
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Command failed: {str(e)}", "suggestions": ["Check your internet connection", "Ensure you have required system packages installed", "Verify user permissions for the workspace directory"]}
    except Exception as e:
        return {"status": "error", "message": f"Linux setup failed: {str(e)}", "suggestions": ["Ensure Python is properly installed", "Check available disk space", "Verify system packages are up to date"]}
...
'''
    ...
    print("To activate the Zephyr environment in future sessions, run: source zephyr_env.sh")
...
def _setup_macos_environment(
    ...
    try:
        ...
        return {"status": "success", "message": "Zephyr environment set up successfully on macOS", "venv_path": venv_path}
        
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Command failed: {str(e)}", "suggestions": ["Check your internet connection", "Ensure you have Xcode Command Line Tools installed", "Verify user permissions for the workspace directory"]}
    except Exception as e:
        return {"status": "error", "message": f"macOS setup failed: {str(e)}", "suggestions": ["Ensure Python is properly installed", "Check available disk space", "Verify Homebrew is up to date"]}
...
def _check_windows_dependencies() -> Dict[str, Any]:
    ...
    try:
        ...
        if shutil.which("git") is None:
            return {"status": "error", "message": "Git is not installed. Please install Git from https://git-scm.com/download/win"}
        git_version = subprocess.check_output(["git", "--version"], universal_newlines=True).strip()
        print(f"Git: {git_version} (OK)")

        # Check CMake
        if shutil.which("cmake") is None:
            return {"status": "error", "message": "CMake is not installed. Please install CMake 3.20.1 or higher from https://cmake.org/download/"}
        ...
        if len(cmake_parts) >= 3:
            cmake_major, cmake_minor, cmake_patch = int(cmake_parts[0]), int(cmake_parts[1]), int(cmake_parts[2])
            if (cmake_major, cmake_minor, cmake_patch) < (3, 20, 1):
                return {"status": "error", "message": "CMake 3.20.1 or higher is required"}
        
        # Check Ninja (optional but recommended)
        ninja_available = shutil.which("ninja") is not None
        if ninja_available:
            ninja_version = subprocess.check_output(["ninja", "--version"], universal_newlines=True).strip()
            print(f"Ninja: {ninja_version} (OK)")
        else:
            print("Ninja not found (optional but recommended). Install from https://github.com/ninja-build/ninja/releases")

        return {"status": "success", "message": "All required dependencies are installed"}
        
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Dependency check failed: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"Windows dependency check failed: {str(e)}"}
...
def _check_linux_dependencies() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check Git
        if shutil.which("git") is None:
            return {"status": "error", "message": "Git is not installed. Please install with: sudo apt-get install git"}
        git_version = subprocess.check_output(["git", "--version"], universal_newlines=True).strip()
        print(f"Git: {git_version} (OK)")

        # Check CMake
        if shutil.which("cmake") is None:
            return {"status": "error", "message": "CMake is not installed. Please install CMake 3.20.1 or higher"}
        cmake_version = subprocess.check_output(["cmake", "--version"], universal_newlines=True).split()[2]
        print(f"CMake version: {cmake_version}")
        ...
        if shutil.which("ninja") is None:
            return {"status": "error", "message": "Ninja is not installed. Please install with: sudo apt-get install ninja-build"}
        ninja_version = subprocess.check_output(["ninja", "--version"], universal_newlines=True).strip()
        print(f"Ninja: {ninja_version} (OK)")
        ...
        for tool in essential_tools:
            if shutil.which(tool) is None:
                return {"status": "error", "message": f"{tool} is not installed. Please install build-essential package"}
        ...
        return {"status": "error", "message": f"Linux dependency check failed: {str(e)}"}
...
def _check_macos_dependencies() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check Git
        if shutil.which("git") is None:
            return {"status": "error", "message": "Git is not installed. Please install Xcode Command Line Tools or Git from https://git-scm.com/"}
        git_version = subprocess.check_output(["git", "--version"], universal_newlines=True).strip()
        print(f"Git: {git_version} (OK)")

        # Check Xcode Command Line Tools
        try:
            subprocess.check_output(["xcode-select", "--version"], universal_newlines=True)
            print("Xcode Command Line Tools: Installed (OK)")
        except (subprocess.CalledProcessError, FileNotFoundError):
            return {"status": "error", "message": "Xcode Command Line Tools are not installed. Please run: xcode-select --install"}

        # Check CMake
        if shutil.which("cmake") is None:
            return {"status": "error", "message": "CMake is not installed. Please install CMake 3.20.1 or higher using Homebrew or from https://cmake.org/download/"}
        cmake_version = subprocess.check_output(["cmake", "--version"], universal_newlines=True).split()[2]
        print(f"CMake version: {cmake_version}")
        ...
        # Check Ninja
        if shutil.which("ninja") is None:
            return {"status": "error", "message": "Ninja is not installed. Please install with Homebrew: brew install ninja"}
        ninja_version = subprocess.check_output(["ninja", "--version"], universal_newlines=True).strip()
        print(f"Ninja: {ninja_version} (OK)")
        ...
        return {"status": "error", "message": f"macOS dependency check failed: {str(e)}"}
        ...
        for path in vs_paths:
            if os.path.isdir(path):
                ...
                for item in os.listdir(path):
                    if item.startswith(('2019', '2022', '2024')) and os.path.isdir(os.path.join(path, item)):
                        return True
        
        # Check for BuildTools specifically
        build_tools_path = os.path.join(os.environ.get('ProgramFiles(x86)', 'C:\Program Files (x86)'), 
...
def _setup_windows_env_vars(workspace_path: str, venv_path: str) -> None:
    ...
    try:
        ...
        with open(env_batch_file, 'w') as f:
            ...
            f.write(f"set ZEPHYR_BASE={os.path.join(workspace_path, 'zephyr').replace('\\', '\\\\')}\n")
            f.write("set ZEPHYR_TOOLCHAIN_VARIANT=zephyr\n")
            f.write(f"set ZEPHYR_SDK_INSTALL_DIR={os.path.join(workspace_path, 'tools', 'zephyr-sdk').replace('\\', '\\\\')}\n")
            f.write("echo Activating Python virtual environment...\n")
            f.write(f"call {os.path.join(venv_path, 'Scripts', 'activate').replace('\\', '\\\\')}\n")
            f.write("echo Environment variables set and virtual environment activated.\n")
...
def _install_windows_sdk(workspace_path: str, sdk_version: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": "Zephyr SDK installation instructions provided", "sdk_path": sdk_dir}
        
    except Exception as e:
        return {"status": "error", "message": f"Windows SDK installation failed: {str(e)}"}
...
def _install_linux_sdk(workspace_path: str, sdk_version: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "message": "Zephyr SDK installed successfully", "sdk_path": sdk_dir}
        
    except subprocess.CalledProcessError as e:
        return {"status": "error", "message": f"Command failed: {str(e)}"}
    except Exception as e:
        return {"status": "error", "message": f"Linux SDK installation failed: {str(e)}"}
...
def _install_macos_sdk(workspace_path: str, sdk_version: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"macOS SDK installation failed: {str(e)}"}
...
def _get_environment_details(workspace_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get West version
        try:
            west_version = subprocess.check_output(["west", "--version"], universal_newlines=True).strip()
...
def validate_zephyr_environment(workspace_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.isdir(workspace_path):
            return {"status": "error", "message": f"Workspace directory not found: {workspace_path}"}
        
        # Check if Zephyr directory exists
        zephyr_dir = os.path.join(workspace_path, "zephyr")
        details["zephyr_directory_exists"] = os.path.isdir(zephyr_dir)
        if not details["zephyr_directory_exists"]:
            return {"status": "error", "message": f"Zephyr directory not found in workspace"}
        ...
        if not details["west_initialized"]:
            warnings.append("West configuration not found. Environment may not be fully initialized.")
        ...
        if missing_packages:
            warnings.append(f"Missing Python dependencies: {', '.join(missing_packages)}")
        else:
        ...
        if details["west_initialized"]:
            try:
                os.chdir(workspace_path)
                subprocess.check_output(["west", "list"], universal_newlines=True, stderr=subprocess.STDOUT)
        ...
        if details["sdk_directory_exists"]:
            ...
            sdk_tools = os.path.join(sdk_dir, "sysroots", "x86_64-pokysdk-linux", "usr", "bin")
        ...
        # Check for CMake
        try:
            cmake_version = subprocess.check_output(["cmake", "--version"], universal_newlines=True).strip()
        ...
        details["zephyr_directory"] = zephyr_dir if details["zephyr_directory_exists"] else None
...
Examples:
  ...
  Force overwrite existing workspace:
    ...
    parser.add_argument("--version", default="latest", help="Zephyr version to install (e.g., v3.5.0, main)")
    parser.add_argument("--no-sdk", action="store_false", dest="install_sdk", help="Skip SDK installation")
    parser.add_argument("--sdk-version", default="0.16.8", help="Version of the Zephyr SDK to install")
    ...
    try:
        ...
        if result['status'] == 'success':
            ...
            if platform.system() == "Windows":
                env_script = os.path.join(os.path.abspath(args.workspace), "zephyr_env.cmd")
                print(f"1. Run the environment setup script: {env_script}")
            else:
                env_script = os.path.join(os.path.abspath(args.workspace), "zephyr_env.sh")
            ...
            print("3. For more information, visit: https://docs.zephyrproject.org/latest/getting_started/index.html")
