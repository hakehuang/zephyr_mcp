...
def _west_init_core(
    ...
    if repo_url:
        ...
        if not git_url_pattern.match(repo_url):
            return {
                "status": "error",
                "suggestion": "仓库URL格式不正确，请提供有效的Git仓库URL",
            }
    ...
    if branch:
        ...
        if any(char in branch for char in invalid_chars):
            return {
                "status": "error",
                "suggestion": "分支名称包含无效字符，请使用合法的分支名称",
            }
    ...
    if project_dir:
        ...
        if os.path.exists(project_dir):
            ...
            if os.listdir(project_dir):
                return {
                    "status": "warning",
                    "suggestion": "项目目录已存在且不为空，可能会覆盖现有内容",
                }
    ...
    if not tools_status.get("west", False):
        return {
            "status": "error",
            "suggestion": "未找到west工具，请确保已正确安装Zephyr SDK",
        }

    return {"status": "success", "suggestion": "参数验证通过"}


def validate_west_init_params(
    repo_url: Optional[str] = None,
    branch: Optional[str] = None,
    project_dir: Optional[str] = None,
    auth_method: str = "embedded",
) -> Dict[str, Any]:
    """
    ...
    if auth_method not in valid_auth_methods:
        return {
            "status": "error",
            "suggestion": f"无效的认证方法，支持的方法: {', '.join(valid_auth_methods)}",
        }
