...
class ToolWrapper:
    ...
    def wrap_tool(tool_func: Callable) -> Dict[str, Any]:
        ...
        # 提取参数信息
        tool_info["parameters"] = ToolWrapper._extract_parameters(
            tool_func, signature, type_hints
        )
        ...
        # 提取异常处理信息和模块
        tool_info["exception_handling"] = ToolWrapper._extract_exception_handling(
            tool_func
        )
        tool_info["module"] = str(
            tool_func.__module__
        )  # 转换为字符串避免JSON序列化错误
    ...
    def _extract_parameters(
        tool_func: Callable, signature: inspect.Signature, type_hints: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        ...
        for param_name, param in signature.parameters.items():
            ...
            description = param_descriptions.get(
                param_name, f"参数 {param_name} 的描述"
            )
                ...
                for line in param_section.split("\n"):
                    ...
                    if line:
                        ...
                        # 处理 param_name: description 格式
                            ...
                            keyword in line
                            for keyword in ["returns", "返回", "参数", "args"]
                        ):
                            parts = line.split(":", 1)
                            if len(parts) >= 1:
                                param_name = parts[0].strip()
        ...
        if not param_descriptions:
            ...
            for line in lines:
                ...
                if ":" in line and not any(
                    ...
                    for keyword in ["returns", "返回", "description", "描述"]
                ):
                    parts = line.split(":", 1)
                    if len(parts) >= 1:
                        ...
                        if re.match(r"^\w+$", param_candidate):
                            param_descriptions[param_candidate] = (
                                parts[1].strip() if len(parts) > 1 else ""
                            )
    ...
    def _extract_returns(
        tool_func: Callable, type_hints: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        Extract return value information
        提取返回值信息
    ...
    def _extract_description(tool_func: Callable) -> str:
        ...
        for line in docstring.split("\n"):
            ...
            # 检查是否到达参数部分
            if any(
                stripped_line.lower().startswith(keyword)
                for keyword in [
                    "parameters:",
                    "参数说明:",
                    "args:",
    ...
    def _extract_exception_handling(tool_func: Callable) -> str:
        ...
        # 查找异常处理部分
        patterns = [r"Raises:\s*(.*?)$", r"异常处理:\s*(.*?)$", r"异常:\s*(.*?)$"]
