...
from .common_tools import run_command, format_error_message, is_git_repository
...
def _fetch_branch_or_pr_internal(project_dir: str, branch_name: Optional[str], 
                               pr_number: Optional[int], remote_name: str) -> Dict[str, Any]:
    """
    ...
    if pr_number is not None:
        ...
        if checkout_result["status"] != "success":
            ...
            track_cmd = ["git", "checkout", "-b", branch_name, f"{remote_name}/{branch_name}"]
...
def _git_rebase_internal(project_dir: str, source_branch: str, onto_branch: Optional[str], 
                        interactive: bool, force: bool) -> Dict[str, Any]:
    """
    ...
    if rebase_result["status"] != "success":
        ...
        # 检查是否是冲突错误
        if "CONFLICT" in rebase_result["stderr"] or "CONFLICT" in rebase_result["stdout"]:
