...
class ZephyrMCPAgent:
    ...
    def __init__(self, config_path: str = "config.json"):
        ...
        if self.current_language not in self.supported_languages:
            self.logger.warning(f"Language '{self.current_language}' not supported, defaulting to 'zh'")
    ...
    def _setup_logger(self, level="INFO"):
        ...
        def record_factory(*args, **kwargs):
            ...
            record.trace_id = trace_id if trace_id is not None else '-'  # 使用传入的trace_id或默认值 / Use provided trace_id or default value
                    ...
                    if span:
                        with self.otel_tracer.start_as_current_span(f"register_tool_{tool_name}") as tool_span:
                            ...
                            if hasattr(self.agent, 'add_tool'):
                                ...
                                self.agent.register_tool(tool)  # 兼容旧版本 / Compatible with older versions
    ...
    def switch_language(self, language: str) -> bool:
        ...
        if language not in self.supported_languages:
            self.logger.error(f"Language '{language}' is not supported. Supported languages: {self.supported_languages}")
            return False
    ...
    def generate_bilingual_documentation(self, output_file: str = "./tools_documentation_bilingual.md"):
        ...
        try:
            ...
            with open(output_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"生成时间 / Generated at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
                ...
                for category, tool_names in categories.items():
                    if tool_names:
                        ...
                        f.write(f"## {category_name_zh} / {category_name_en} ({len(tool_names)})\n\n")
                        ...
                        for tool_name in tool_names:
                            ...
                            if tool_info.get('parameters'):
                                ...
                                for param_name, param_info in tool_info['parameters'].items():
                                    param_desc = param_info.get('description', '无描述 / No description')
                                    f.write(f"- `{param_name}`: {param_desc}\n")
                                f.write("\n")
                            
                            if tool_info.get('returns'):
                                return_desc = tool_info['returns'].get('description', '无描述 / No description')
